...
impl Calculator {
    ...
    pub fn compute_aggregate_rates(
        ...
        fn aggregate_inputs(
            ...
            let (producers, inputs) = Calculator::compute_required_rates(&ingredient, required_rate, recipes);
            agg_map
                .entry(ingredient)
        ...
        let mut visit_num = 0;
        aggregate_inputs(self.output_ingredient.clone(), self.output_rate, &mut visit_num, &mut aggregate_rates, known_recipes);
        ...
        let sorted_rates = unsorted_rates.into_iter().map(|(_, val)| val).collect();
        sorted_rates
    }
}